...
export default function HistoricalTweets() {
    ...
    useEffect(() => {
        fetch(`${SERVER}/api/fields/suburb?time=historical`, {
          method: 'GET'
        }).then(
          ...
          data => {
            ...
            if (FIELDS.length > 1){
              setSuburbs(["FIELDS[0]", "Melbourne"]);
            } else{
              setSuburbs(["Melbourne"]);
            }
          }
        )
      }, []);
    ...
    useEffect(() => {
        let link = `${SERVER}/api/overview/line_historical`;
        if (suburbs.length > 0) {
            link = link + '?';
  ...
  return (
    <Card>
      <CardHeader title="Sentiment Index (Historical)" />
