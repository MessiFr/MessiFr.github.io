...
export default function PieChartTraffic() {
  ...
  useEffect(() => {
    fetch(`${SERVER}/api/fields/traffic`, {
      method: 'GET'
    }).then(
  ...
  useEffect(() => {
    fetch(`${SERVER}/api/overview/pie/traffic?indicator=${indicator}`, {
      method: 'GET',
    //   body: dataBody
    }).then(
      response => response.json()
  ...
  return (
    <Card>
      <CardHeader title="Traffic Condition (Top 10 Suburbs)" />
